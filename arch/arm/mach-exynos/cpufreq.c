			  ...
			  unsigned int relation)
{
	...
	//printk("***************%s: target index is %d vs old %d --freq is %d vs old %ld***********\n",
	//	__func__,index,old_index,target_freq,freqs.old);
	...
	if (!exynos_cpufreq_lock_disable && (index > g_cpufreq_lock_level)){
		index = g_cpufreq_lock_level;
			...
			//printk("%d  lock level is  %d \n",i,g_cpufreq_lock_val[i]);
			//}
		}
	...
	if (!exynos_cpufreq_lock_disable && (index < g_cpufreq_limit_level))
		index = g_cpufreq_limit_level;
			//for (i = 0; i < DVFS_LOCK_ID_END; i++) {
			//	printk("%d  limit level is  %d \n",i,g_cpufreq_limit_val[i]);
			//}
...
static int __init exynos_cpufreq_init(void)
{
	...
	printk("***************%s-----(lock: %d; limit : %d)***********\n",__func__,g_cpufreq_lock_level,g_cpufreq_limit_level);
