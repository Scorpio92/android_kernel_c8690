...
static struct map_desc exynos4_iodesc[] __initdata = {
	{
		...
		.type           = MT_DEVICE,
                ...
                .virtual        = (unsigned long)S5P_VA_QOS_CAM,
                .pfn            = __phys_to_pfn(EXYNOS4_PA_QOS_CAM),
                .length         = SZ_4K,
                .type           = MT_DEVICE,
	},
	...
	{
		.virtual        = (unsigned long)S3C_VA_KMSG_DUMP,
		.pfn            = __phys_to_pfn(PANIC_MEM_POOL_ADDR_PA),
		.length         = 0x100000, //  1M
		.type           = MT_MEMORY_NONCACHED,
	},{
		.virtual        = (unsigned long)S3C_VA_LOGGER1_DUMP,
		.pfn            = __phys_to_pfn(PANIC_MEM_POOL_LOGGER_ADDR1_PA),
		.length         = 0x100000, //  1M
		.type           = MT_MEMORY_NONCACHED,
	},{
		.virtual        = (unsigned long)S3C_VA_LOGGER2_DUMP,
		.pfn            = __phys_to_pfn(PANIC_MEM_POOL_LOGGER_ADDR2_PA),
		.length         = 0x100000, //  1M
		.type           = MT_MEMORY_NONCACHED,
	},
...
//#define FACOTRY_RESET_MODE1 "?"//not defined now..
#endif
static void exynos4_sw_reset(const char *cmd)
{
// Cellon modify start, Ted Shi, 2012/12/06, for power up directly after factory reset
//LY: remove strcmp : it will casued system crash when reboot on 4412 EVT1.1 
//	int count = 3;
	int info2_value;
	int mode = 0;
	//printk(KERN_INFO"[xx]go to reset system\n");
	//if (cmd)//(!strcmp(cmd,FACOTRY_RESET_MODE))
	//devin
	if (cmd)
	{
		if ( cmd[0] == 'r' &&
...
//avoid reboot entry charger mode	
	__raw_writel(0x01, S5P_INFORM7); 

		info2_value = __raw_readl(S5P_INFORM2);
	if(mode){
		info2_value = ((info2_value & (~0xffff)) | mode);
	}else{
		info2_value &= (~0xff);
	}
//	while (count--) {
//		__raw_writel(mode, S5P_INFORM5); //mj : factory reset mode 
		__raw_writel(info2_value, S5P_INFORM2); 
		__raw_writel(0x1, S5P_SWRESET);
//		mdelay(500);
//	}
// Cellon modify end, Ted Shi, 2012/12/06
}
...
#else
		__raw_writel((value | (1<< 26)), S5P_MASK_WDT_RESET_REQUEST);//zxz for ISP. disable A5 wdt
#endif
	}
