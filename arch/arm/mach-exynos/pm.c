...
void exynos4_cpu_suspend(void)
{
	...
	if ((!cpu_is_exynos4210()) && (exynos4_is_c2c_use())) {
		...
		tmp = __raw_readl(S5P_CLKGATE_IP_DMC);
		tmp &= ~(0x1 << 17);
		tmp |= (0x1 << 4);
		__raw_writel(tmp, S5P_CLKGATE_IP_DMC);
		...
		tmp = __raw_readl(S5P_CLKSRC_DMC);
		tmp &= ~S5P_CLKSRC_DMC_MASK;
		tmp |= ((0x6 << 16)|(0x1 << 12));
		__raw_writel(tmp, S5P_CLKSRC_DMC);
	}
