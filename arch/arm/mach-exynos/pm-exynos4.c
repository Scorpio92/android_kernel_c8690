...
static int tc4_dvt_pm_prepare(void)
{
	int ret = 0;
	//Robin, only add for TC4 DVT.
	//If the regulator in EVT is available, this also can for EVT.
#if defined(CONFIG_REGULATOR) && defined(CONFIG_REGULATOR_S5M8767)
...
static void exynos4_pm_resume(void)
{
	...
	printk("wakeup from sleep 0x%08x\n",tmp);
