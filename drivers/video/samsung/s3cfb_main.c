...
// add by justy.yang 20121108 for LCD power domain
#include <plat/pd.h>
...
static irqreturn_t s3cfb_irq_frame(int irq, void *dev_id)
{
	...
	wake_up(&fbdev[0]->wq);
	//modify by isuma
	schedule_work(&vsync_work);//enable work
...
void s3cfb_early_suspend(struct early_suspend *h)
{
	...
	/* disable the power domain */
	printk( "s3cfb - disable power domain\n");
	pm_runtime_put_sync(&pdev->dev);
	/* add following statement because runtime pm of PD_LCD0 */
	/* disabled for showing logo in bootloader 				 */
	...
	if( IS_ERR_OR_NULL(lcd_regulator_ldo13) )
	{
		printk( "error on vddioperi_18 regulator disable : lcd_regulator is null\n");
		return;
	}
	regulator_force_disable(lcd_regulator); 	//yulu
//	regulator_force_disable(lcd_regulator_ldo13); 	//jacob
...
void s3cfb_late_resume(struct early_suspend *h)
{
	...
	if( IS_ERR_OR_NULL(lcd_regulator_ldo13) )
	{
		printk("error on vddioperi_18 regulator enable : lcd_regulator is null\n");
		return;
	}
	if( IS_ERR_OR_NULL(lcd_regulator) )
	{
		printk("error on lcd33 regulator enable : lcd_regulator is null\n");
		return;
	}
	regulator_enable(lcd_regulator_ldo13); 	//jacob
	regulator_enable(lcd_regulator); 	//yulu
