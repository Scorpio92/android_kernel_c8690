...
#ifdef CONFIG_PM
static int lsm330dlc_acc_early_resume(struct early_suspend *handler);
static int lsm330dlc_acc_early_suspend(struct early_suspend *handler);
#endif
...
{
	struct lsm330dlc_acc_data *acc = input_get_drvdata(input);
...
{
	struct lsm330dlc_acc_data *acc = input_get_drvdata(dev);
		...
		const struct i2c_device_id *id)
{
	...
	if(acc->pdata->gpio_int1 >= 0){
		...
		acc_printk("%s: %s has set irq1 to irq: %d "
							"mapped on gpio:%d\n",
			LSM330DLC_ACC_DEV_NAME, __func__, acc->irq1,
							acc->pdata->gpio_int1);
	}

	if(acc->pdata->gpio_int2 >= 0){
		acc->irq2 = gpio_to_irq(acc->pdata->gpio_int2);
		acc_printk("%s: %s has set irq2 to irq: %d "
							"mapped on gpio:%d\n",
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	acc->early_suspend.suspend = lsm330dlc_acc_early_suspend;
	acc->early_suspend.resume = lsm330dlc_acc_early_resume;
...
{
//modify by D.Z cellon 20121119
#if 0
	struct lsm330dlc_acc_data *acc = i2c_get_clientdata(client);

//	acc_printk("lsm330dlc_acc_resume___\n");
	if (acc->on_before_suspend)
	{
//		acc_printk("lsm330dlc_acc_resume--------22\n");
		return lsm330dlc_acc_enable(acc);
	}
#endif
//modify end
...
#if 0
	struct lsm330dlc_acc_data *acc = i2c_get_clientdata(client);
//	acc_printk("lsm330dlc_acc_suspend___\n");
...
//add by D.Z cellon 20121119
static int lsm330dlc_acc_early_resume(struct early_suspend *handler)
{
	struct lsm330dlc_acc_data *acc;
	acc = container_of(handler, struct lsm330dlc_acc_data, early_suspend);
	acc_printk("lsm330dlc_acc_late_resume___\n");
	if (acc->on_before_suspend)
	{
		return lsm330dlc_acc_enable(acc);
	}
...
static int lsm330dlc_acc_early_suspend(struct early_suspend *handler)
{
	struct lsm330dlc_acc_data *acc;
	acc = container_of(handler, struct lsm330dlc_acc_data, early_suspend);
	acc_printk("lsm330dlc_acc_early_suspend___\n");
	acc->on_before_suspend = atomic_read(&acc->enabled);
	return lsm330dlc_acc_disable(acc);
}
//add end
...
{
	acc_printk("%s accelerometer driver: init\n",
						LSM330DLC_ACC_DEV_NAME);
...
{
#ifdef DEBUG
	acc_printk("%s accelerometer driver exit\n",
						LSM330DLC_ACC_DEV_NAME);
#endif /* DEBUG */
