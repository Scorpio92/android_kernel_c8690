...
#ifdef CONFIG_PM
static int lsm330dlc_gyro_early_resume(struct early_suspend *handler);
static int lsm330dlc_gyro_early_suspend(struct early_suspend *handler);
#endif
...
//add by D.Z cellon 20121119
#ifdef CONFIG_HAS_EARLYSUSPEND
	gyro->early_suspend.suspend = lsm330dlc_gyro_early_suspend;
	gyro->early_suspend.resume = lsm330dlc_gyro_early_resume;
...
//	struct i2c_client *client = to_i2c_client(dev);
//	struct lsm330dlc_data *data = i2c_get_clientdata(client);
	struct lsm330dlc_data *data;
	u8 buf[2];
	data = container_of(handler, struct lsm330dlc_data, early_suspend);
//#if DEBUG
//	dev_info(&client->dev, "suspend\n");
//#endif /* DEBUG */
	gyr_printk("%s\n", __func__);
	if (atomic_read(&data->enabled)) {
		...
		if (data->polling_enabled) {
			pr_info("polling disabled\n");
			cancel_delayed_work(&data->input_poll_dev->work);
		}
#ifdef SLEEP
		err = lsm330dlc_register_update(data, buf, CTRL_REG1,
				0x0F, (ENABLE_NO_AXES | PM_NORMAL));
...
static struct dev_pm_ops lsm330dlc_pm = {
	.suspend = NULL,//lsm330dlc_suspend,
	.resume = NULL,//lsm330dlc_resume,
};
//modify end
