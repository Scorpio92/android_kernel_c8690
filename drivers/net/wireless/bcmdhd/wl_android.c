...
strtol(const char *nptr)
{
	...
	if (isdigit(c))
		...
		printk("%s: chranslate wrong char \n",__func__);
		return -1;
	}
	return (c);
}
...
// Cellon add start, Ted Shi, 2012/10/20, for add wifi MAC
	...
	*/
	if (mac_addr[0] != '\0') {
		...
		wifi_mac[2] = (strtol(&mac_addr[4]) << 4) | strtol(&mac_addr[5]);
		wifi_mac[3] = (strtol(&mac_addr[6]) << 4) | strtol(&mac_addr[7]);
		wifi_mac[4] = (strtol(&mac_addr[8]) << 4) | strtol(&mac_addr[9]);
		wifi_mac[5] = (strtol(&mac_addr[10]) << 4) | strtol(&mac_addr[11]);

		bcopy(wifi_mac, buf, sizeof(wifi_mac));
		return 0;
	}
// Cellon add end, Ted Shi, 2012/10/20
	return -EOPNOTSUPP;
