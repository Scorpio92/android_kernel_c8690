...
static int mali_pm_suspend(struct device *dev)
{
	...
	_mali_osk_lock_signal(lock, _MALI_OSK_LOCKMODE_RW);
#ifdef CONFIG_REGULATOR //Robin Wang, Let vdd_g3d = 1.1v temp. The best solution is get from the ASV table 
	mali_regulator_set_voltage(1100000,1100000);
#endif
...
extern struct device_attribute dev_attr_mali_lock;
...
int _mali_dev_platform_register(void)
{
	...
	if (!err) 
	{
		err = device_create_file(&mali_gpu_device.dev,&dev_attr_mali_lock);
		 if (err){
			MALI_PRINT(("create mali_lock sysfs failed\n")); 
		 }
