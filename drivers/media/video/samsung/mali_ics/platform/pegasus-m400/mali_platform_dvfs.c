...
int mali_dvfs_bottom_lock_pop();
/* sysfs for mali frequency lock/unlock */

/**
 * show_mali_bottom_lock
 */
static ssize_t show_mali_bottom_lock(struct device *dev,
 struct device_attribute *attr, char *buf)
{
 return sprintf(buf, "Mali bottom freq is locked upon %d\n", bottom_lock_step); 
}


/**
 * store_mali_bottom_lock
 */
static ssize_t store_mali_bottom_lock(struct device *dev, struct device_attribute *attr,
 const char *buf, size_t count)
{
 int ret = -EINVAL;
 char cmd[7];

 ret = sscanf(buf, "%7s", cmd);
 ...
 {
 mali_dvfs_bottom_lock_push(3);
 MALI_PRINT(("gpu freq is locked at 440MHz\n"));
 }
...
MALI_PRINT(("Unkown cmd\n"));

return strnlen(buf, PAGE_SIZE);

}

DEVICE_ATTR(mali_lock, 0666, show_mali_bottom_lock, store_mali_bottom_lock);
EXPORT_SYMBOL(dev_attr_mali_lock);
#endif
