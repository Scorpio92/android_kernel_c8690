...
static ssize_t rmi_fn_01_productid_show(struct device *dev,
					struct device_attribute *attr,
					char *buf);
static ssize_t rmi_fn_01_configid_show(struct device *dev,
					struct device_attribute *attr,
					char *buf);
...
static struct device_attribute fn_01_attrs[] = {
	...
	__ATTR(productid, RMI_RO_ATTR,
        ...
        __ATTR(configid, RMI_RO_ATTR,
	       rmi_fn_01_configid_show, rmi_store_error),
...
//	return snprintf(buf, PAGE_SIZE, "0x%02x 0x%02x 0x%02x 0x%02x\n", data->config_id[0],data->config_id[1],data->config_id[2],data->config_id[3]);
...
static int rmi_f01_initialize(struct rmi_function_container *fc)
{
	...
	retval = rmi_read(rmi_dev, 0xEB, &configaddr);
			if (retval < 0) {
				dev_err(&fc->dev,
					"Failed to read config addr 0xeb \n");
				return retval;
			}
	retval = rmi_read_block(rmi_dev,configaddr, data->config_id,
					sizeof(data->config_id));
		if (retval < 0) {
			dev_err(&fc->dev,
				"Failed to read config ID \n");
			return retval;
		}
