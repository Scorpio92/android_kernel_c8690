...
// Cellon add start, Ted Shi, 2012/12/06, for alarm auto test
//#define AUTO_ALARM
#undef AUTO_ALARM
#ifdef AUTO_ALARM
#define ALARM_PERIOD 60
#endif
// Cellon add end, Ted Shi, 2012/12/06
...
int rtc_set_alarm_for_poweroff(struct timespec new_time)
{
	int ret;
    ...
    memset((void *)&alrm, 0, sizeof(struct rtc_wkalrm));
    memcpy(&alrm.time, &rtc_new_rtc_time, sizeof(struct rtc_time));
    
    alrm.enabled = 1;
    alrm.pending = 1;
    ret = rtc_set_alarm(alarm_rtc_dev, &alrm);
	if (ret < 0)
		printk("rtc_set_alarm_for_poweroff: "
...
extern void s5m_read_reg_status();
...
// Cellon add start, Ted Shi, 2012/12/06, for alarm auto test
#ifdef AUTO_ALARM
		rtc_alarm_time = rtc_current_time + ALARM_PERIOD;
		rtc_time_to_tm(rtc_alarm_time, &rtc_alarm.time);
		rtc_set_alarm(alarm_rtc_dev, &rtc_alarm);
#endif
// Cellon add end, Ted Shi, 2012/12/06
...
static int alarm_shutdown(struct platform_device *plat_dev)
{
	struct rtc_wkalrm alarm;
