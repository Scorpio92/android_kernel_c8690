...
// Cellon add start, Ted Shi, 2011/08/08, for porting bcm4330 wifi
	#ifdef CONFIG_BCMDHD_WIFI
	/* for BCM WIFI */
	if (pdata->pm_flags)
		host->mmc->pm_flags |= pdata->pm_flags;
	#endif
// Cellon add end, Ted Shi, 2011/08/08	
...
// Cellon add start, Ted Shi, 2012/09/29, for wifi resume kernal crash
//	if(!(host->mmc->caps & MMC_CAP_NONREMOVABLE)){//lisw hotplug during suspend
	if((!(host->mmc->caps & MMC_CAP_NONREMOVABLE)) && (host->mmc->index != 2)){//lisw hotplug during suspend
		int status = gpio_get_value(sc->ext_cd_gpio);
		if (sc->pdata->ext_cd_gpio_invert)
			status = !status;
		sdhci_s3c_notify_change(sc->pdev, status);
...
// Cellon modify end, Ted Shi, 2012/09/29
	if(!strcmp(mmc_hostname(host->mmc),"mmc2")){
//		printk("%s: mmc2 realy enable irq %d\n",__func__,WLAN_OOB_IRQ);
		enable_irq_bcm4330(WLAN_OOB_IRQ);
	}
	return 0;
}
