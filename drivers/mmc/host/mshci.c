				...
				unsigned int clock, u32 ddr)
{
	...
	if (clock == host->clock){//lisw ms : for too frequencily change clock
	return;
	}
	if (clock > host->max_clk) {
		return;
	}
	//printk("%s:%s, %d", mmc_hostname(host->mmc),__func__, clock);
...
/*	else {
	...
	mshci_writel(host, 0x01, MSHCI_CLKDIV);
	mshci_writel(host, 0x00, MSHCI_CLKSRC);
