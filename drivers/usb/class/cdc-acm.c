...
#define ACM_SUSPEND_UNLOCK_DELAY msecs_to_jiffies(500)//(5*HZ)

static inline void acm_wake_lock_init(struct acm *acm)
{
	wake_lock_init(&acm->pm_lock, WAKE_LOCK_SUSPEND, "cdc-acm");
	//wake_lock_init(&acm->dormancy_lock, WAKE_LOCK_SUSPEND, "acm-dormancy"); //commented by ylk  20120705
...
static int acm_initiated_resume(struct acm *acm)
	{
		...
		while(suspending_flag){
			if(count++>5){
				printk("waiting for system suspend timeout! %s()",__func__);
				break;
			}
			else {
	...
	retry:
			...
			case RPM_SUSPENDED:
				...
				case MC_HOST_HIGH:
					...
					break;
				case MC_CP_RESET:
					printk("waiting for system go to suspended state!\n");
					break;
				}
				if (spin2-- == 0) {
					dev_info(&udev->dev,
...

{
	...
	//	wb->use = 0;
		spin_unlock_irqrestore(&acm->write_lock, flags);
		//////wake_lock_timeout_data(acm->parent); //mask by samsung 20121128
...
static void acm_tty_close(struct tty_struct *tty, struct file *filp)
{
	...
	if (!acm){
		printk("%s *******acm error : not available*******\n",__func__);
		return;
	}
	mutex_lock(&open_mutex);
	if (tty_port_close_start(&acm->port, tty, filp) == 0) {
		...
		mutex_unlock(&open_mutex);
		printk("%s close ttyACM : tty_port_close_start()==0 \n",__func__);
		return;
	}
	acm_port_down(acm);
	...
	tty_port_tty_set(&acm->port, NULL);
	mutex_unlock(&open_mutex);
	printk("%s*****close ttyACM successful*******\n",__func__);
}
...
static void acm_runtime_start(struct work_struct *work)
{
	...
	//add by justy.yang 20121011 for issue of modem enumeration fail start
	//printk("debug : acm_runtime_start \n");
	pm_runtime_set_active(s5p_dev);
	pm_runtime_enable(s5p_dev);
	//add by justy.yang 20121011 for issue of modem enumeration fail end
...
//static struct workqueue_struct *cdc_acm_workqueue;
static struct delayed_work cdc_acm_resume_work;
static struct acm *acm_rumtime;
static void cdc_acm_do_rumtime(struct work_struct *work)
{
	//printk("cdc_acm_do_runtime\n");
	acm_initiated_resume(acm_rumtime);
	usb_autopm_get_interface(acm_rumtime->control);
	usb_autopm_put_interface(acm_rumtime->control);
}
...
made_compressed_probe:
	...
	pm_runtime_set_autosuspend_delay(&usb_dev->dev,2000);
...
skip_countries:
	...
	//ly to fix suspend/resume issue
 	// INIT_WORK(&cdc_acm_resume_work, cdc_acm_do_rumtime);
	INIT_DELAYED_WORK(&cdc_acm_resume_work, cdc_acm_do_rumtime);	
	//cdc_acm_workqueue = create_singlethread_workqueue("cdc_acm_workqueue");
	acm_rumtime=acm;
	return 0;
alloc_fail7:
	for (i = 0; i < ACM_NW; i++)
...
static int acm_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	if(hsic_entered_l3){
                hsic_entered_l3=0;
                cancel_delayed_work_sync(&cdc_acm_resume_work);
	}
	if(acm->suspended==1)
		return 0;
...
static int acm_resume(struct usb_interface *intf)
{
	...
	if(hsic_entered_l3){
//		hsic_entered_l3=0;
		schedule_delayed_work(&cdc_acm_resume_work, msecs_to_jiffies(500));
	}
//	queue_work(cdc_acm_workqueue, &cdc_acm_resume_work);
err_out:
	mutex_unlock(&acm->mutex);
	return rv;
}
