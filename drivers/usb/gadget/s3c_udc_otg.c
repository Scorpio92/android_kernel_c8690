		...
		int (*bind)(struct usb_gadget *))
{
	...
	udc_enable(dev);
#endif
exynos_usb_mux_change(dev->dev,gpio_get_value(EXYNOS4_GPX0(4))?0:1);
...
static struct platform_driver s3c_udc_driver = {
	...
	//.suspend	= s3c_udc_suspend,// modify for adb can't use when add patch for hsic error when suspend fail 20121128
	//.resume		= s3c_udc_resume, // s3c_udc_resume will call after s5p_ehci_resume 20121128
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= "s3c-usbgadget",
	},
