...
extern int s3c_udc_suspend();
extern int s3c_udc_resume();
...
static int s5p_ehci_suspend(struct device *dev)
{
	...
	unsigned long flags;
	int rc = 0;
	s3c_udc_suspend();// modify for adb can't use when add patch for hsic error when suspend fail 20121128
...
#ifdef CONFIG_SMM6260_MODEM
	if(gpio_get_value(GPIO_IPC_SLAVE_WAKEUP)){
//		printk("Recover the GPIO of GPIO_IPC_SLAVE_WAKEUP.\n");
   ...
   suspending_flag = 0;
   suspended_flag = 0;
   disable_irq(global_mc->irq[1]);
//   printk("disable to hostwakeup irq!\n");
...
static int s5p_ehci_resume(struct device *dev)
{
	...
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
	int rc = 0;
	enable_irq(global_mc->irq[1]);
	printk("enable hostwakeup irq!\n");
#ifdef CONFIG_SMM6260_MODEM	
...
#ifdef CONFIG_SMM6260_MODEM
		//writel(0x03C00000, hcd->regs + 0x90);//mask by xujie 20121211
#endif
...
//	ehci_dbg(ehci, "lost power, restarting\n");
	...
	s3c_udc_resume();// modify for adb can't use when add patch for hsic error when suspend fail 20121128
	return rc;
}
...
#endif
#ifdef CONFIG_USB_SUSPEND
	//mask by justy.yang 20121011 for issue of modem enumeration fail start
	//pm_runtime_set_active(&pdev->dev);
	//pm_runtime_enable(&pdev->dev);
	//mask by justy.yang 20121011 for issue of modem enumeration fail end
#endif
#ifdef CONFIG_GPIO_SMM6260
