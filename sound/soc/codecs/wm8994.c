...
/* DEBUG for dev_dbg ... */
//#define DDEBUG 
//while input the hp in-call ,we will ignore it because of the cache restore,add by tab.
volatile int hp_flag = 0;
//end
...
#define dprintk(x...)
#endif
//#define CALL_LOCK_TEMP
#ifdef CALL_LOCK_TEMP
static struct wake_lock call_wake_lock;
#endif

static int pre_user_value = 0;
//int real_incall_flag = 0;
...
struct workqueue_struct *g_mic_workqueue;
...
static int configure_clock(struct snd_soc_codec *codec)
{
	...
	if (old == new)
	{
		dprintk("Terry:---------- configure_clock----return-------\n");
		return 0;
	}
...
#ifdef TC4_CALL
/*
 * tab: Indicate one voice call duration.
 * 		The code referenced "calling_flag" previously
 *		should be replaced by this one.
 */
static const char *voice_call_flag_name[] = {"NORMAL", "VOICE"};
static const struct soc_enum tc4_voice_call_flag_enum = \
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(voice_call_flag_name), voice_call_flag_name);

static int tc4_set_voice_call_flag(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	int user_value = ucontrol->value.integer.value[0];
	
//	real_incall_flag = user_value;

	return 0;
}

static int tc4_get_voice_call_flag(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{	
	//ucontrol->value.integer.value[0] = real_incall_flag;
	return 0;
}

//0:OFF	1:SPK	2:HP	3:BT
static short voice_path = 0;
static const char *voice_call_path_name[] = {"OFF","VOICE_SPK","VOICE_RCV","VOICE_HP","VOICE_BT","VOICE_NONE","VOIP_BT","VOIP_NORMAL"};
...
// 1 call mode, 0 no call
int tc4_get_call_flg(void)
{
	//printk("tc4_get_call_flg = %d\n",calling_flag);
	return calling_flag;
}

EXPORT_SYMBOL(tc4_get_call_flg);
extern void es305_set_call_mode(int mode);

static int set_in_call_dai_fmt(struct snd_soc_codec *codec, int user_value)
{
	printk("set_in_call_dai_fmt user_value = %d\n", user_value);
	if(user_value == incall_bt_mode || user_value == voip_bt_mode){
		es305_set_call_mode(user_value);//set 305 bt mode 

		snd_soc_update_bits(codec, 0x0310,
		...
		snd_soc_update_bits(codec, WM8994_AIF2_CLOCKING_1,
				    ...
				    WM8994_AIF2CLK_ENA);//enable aif2clk
		
		wm8994_write(codec, WM8994_GPIO_9, 0x4100);//open gpio9 for bt
	}
	else{
		wm8994_write(codec, WM8994_GPIO_9, 0xC101);//close gpio9 to close bt
#if 0
		es305_set_call_mode(mute_uplink_volume);/* For bt noise */
#endif
		snd_soc_update_bits(codec, 0x0310,
			    WM8994_AIF1_BCLK_INV | WM8994_AIF1_LRCLK_INV |
			    WM8994_AIF1_FMT_MASK,
...
static int set_voice_flag_and_fmt(struct snd_soc_codec *codec, int user_value)
{
	int size,i;
	
	printk("set_voice_flag_and_fmt = %d\n", user_value);
	if(user_value > 0)
		calling_flag = 1;
	else
		calling_flag = 0;
	#if 1	
	if(pre_user_value == incall_bt_mode || user_value == incall_bt_mode || pre_user_value == voip_bt_mode || user_value == voip_bt_mode)
		set_in_call_dai_fmt(codec, user_value);
	#else
		if(user_value == 6){
			size = ARRAY_SIZE(incall_bt_reg);
			for(i=0; i < size; i++){
				wm8994_write(codec, incall_bt_reg[i].reg, incall_bt_reg[i].val);
			}
			es305_set_call_mode(user_value);
		}
	#endif
	if(user_value != incall_bt_mode && user_value != voip_bt_mode)
		es305_set_call_mode(user_value);

	pre_user_value = user_value;
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	set_voice_flag_and_fmt(codec, user_value);
	return 0;
}
#endif
...
#ifdef TC4
#ifdef TC4_CALL
SOC_ENUM_EXT("VOICE CALL FLAG", tc4_voice_call_flag_enum,\
		tc4_get_voice_call_flag, tc4_set_voice_call_flag),
...
int late_enable_ev(struct snd_soc_dapm_widget *w,
			  struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	struct wm8994_priv *wm8994 = snd_soc_codec_get_drvdata(codec);
...
EXPORT_SYMBOL_GPL(late_enable_ev);
...
SND_SOC_DAPM_DAC("DAC1L", "AIF1 Playback", WM8994_POWER_MANAGEMENT_5, 1, 0),
SND_SOC_DAPM_DAC("DAC1R", "AIF1 Playback", WM8994_POWER_MANAGEMENT_5, 0, 0),
};
...
SND_SOC_DAPM_AIF_IN_E("AIF1DAC1L", "AIF1 Playback", 0,
		      WM8994_POWER_MANAGEMENT_5, 9, 0, wm8958_aif_ev,
		      SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
SND_SOC_DAPM_AIF_IN_E("AIF1DAC1R", "AIF1 Playback", 0,
...
SND_SOC_DAPM_AIF_OUT("AIF2ADCDAT", "AIF2 Playback", 0, SND_SOC_NOPM, 0, 0), /* lazy way. */

SND_SOC_DAPM_MUX("AIF1DAC Mux", SND_SOC_NOPM, 0, 0, &aif1dac_mux),
SND_SOC_DAPM_MUX("AIF2DAC Mux", SND_SOC_NOPM, 0, 0, &aif2dac_mux),
SND_SOC_DAPM_MUX("AIF2ADC Mux", SND_SOC_NOPM, 0, 0, &aif2adc_mux),

SND_SOC_DAPM_AIF_IN("AIF3DACDAT", "AIF2 Playback", 0, SND_SOC_NOPM, 0, 0), /* keep bt voice call */
SND_SOC_DAPM_AIF_OUT("AIF3ADCDAT", "AIF2 Playback", 0, SND_SOC_NOPM, 0, 0), /* as simple as possible */
...
static const struct snd_soc_dapm_route intercon[] = {
	...
	{ "Left Output Mixer", NULL, "AIF1CLK" },
			  ...
			  unsigned int freq_in, unsigned int freq_out)
{
	...
	struct fll_div fll;
	u16 reg, aif1, aif2;
		dprintk("Terry:---------src=%d,freq_in=%d,freq_out=%d\n",src,freq_in,freq_out);
 ...
 */
static int wm8994_aif2_hw_params(struct snd_pcm_substream *substream,
				 struct snd_pcm_hw_params *params,
				 struct snd_soc_dai *dai)
{
	struct snd_soc_codec *codec = dai->codec;

	printk("wm8994_aif2_hw_params id = %d\n", dai->id);
	
	if (dai->id != 2)
		return 0;
#ifdef CONFIG_PC4_CYIT
	/*set AIF2 data format and clk*/
	snd_soc_update_bits(codec, WM8994_AIF2_CONTROL_1,
			    WM8994_AIF2_WL_MASK, 0x0);
	snd_soc_update_bits(codec, WM8994_AIF2_CONTROL_2,
			    WM8994_AIF1_MONO_MASK, 0x0);
	snd_soc_update_bits(codec, WM8994_AIF2_RATE, WM8994_AIF1_SR_MASK |
			    WM8994_AIF1CLK_RATE_MASK, 0x0033);
	/*set BCLK 512k and 16k LRCLK*/
	snd_soc_update_bits(codec, WM8994_AIF2_BCLK, WM8994_AIF1_BCLK_DIV_MASK, 0x0070);
	snd_soc_update_bits(codec, WM8994_AIF2ADC_LRCLK, WM8994_AIF1DAC_RATE_MASK,
			    0x0020);
#else
	snd_soc_update_bits(codec, WM8994_AIF2_CONTROL_1,
			    WM8994_AIF2_WL_MASK, 0x0000);
	snd_soc_update_bits(codec, WM8994_AIF2_CONTROL_2,
			    WM8994_AIF1_MONO_MASK, WM8994_AIF1_MONO);
	snd_soc_update_bits(codec, WM8994_AIF2_RATE, WM8994_AIF1_SR_MASK |
			    WM8994_AIF1CLK_RATE_MASK, 0x0009);
#endif
	return 0;
}
...
static struct snd_soc_dai_ops wm8994_aif2_dai_ops = {
	.set_sysclk	= wm8994_set_dai_sysclk,
	.set_fmt	= wm8994_set_dai_fmt,
	.hw_params	= wm8994_aif2_hw_params,
	.digital_mute   = wm8994_aif_mute,
	.set_pll	= wm8994_set_fll,
	.set_tristate	= wm8994_set_tristate,
};
...
static struct snd_soc_codec *codec_misc;
#ifdef CONFIG_PM
static int wm8994_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	for (i = 0; i < ARRAY_SIZE(wm8994->fll); i++) {
		//do not close the FLL2 for voice call in AP sleep mode. xujie_2012.03.09
		if(i != 1){  
...
void wm8994_set_micb2(int on)
{
	printk("%s on=%d\n",__func__,on);
	if(on){
		snd_soc_update_bits(g_codec, WM8994_POWER_MANAGEMENT_1,
				    WM8994_MICB2_ENA,
				    WM8994_MICB2_ENA);
		hp_flag = 1;
	}
...
EXPORT_SYMBOL_GPL(wm8994_set_micb2);
...
{
	printk("%s\n",__func__);
	dev_dbg(codec->dev, "wm8994_voicecall_common_setting\n");

	/*GPIO Configuration*/
	wm8994_write(codec, WM8994_GPIO_1,  0xA101);//gpio1 must be set, or mic won't work
	wm8994_write(codec, WM8994_GPIO_2,  0xC100);//mclk2
	/*AIF2 GPIO*/
	wm8994_write(codec, WM8994_GPIO_3,  0xC100);//bclk2	
	wm8994_write(codec, WM8994_GPIO_4,  0xC100);//lrclk2
	wm8994_write(codec, WM8994_GPIO_5,  0xC100);//dacdat2
	wm8994_write(codec, WM8994_GPIO_6,  0xA101);//gpio6
	wm8994_write(codec, WM8994_GPIO_7,  0x4100);//adcdat2
	/*AIF3 GPIO*/
	/*
	...
	*/
	...
	wm8994_write(codec, WM8994_GPIO_8,  0xA100);//dacdat
	wm8994_write(codec, WM8994_GPIO_9,  0xC101);//adcdat tab:mute bt first
	wm8994_write(codec, WM8994_GPIO_10, 0xA100);//sync
	wm8994_write(codec, WM8994_GPIO_11, 0xA100);//clk
#if 0 //we set this in other place
	/*AIF2 rate*/
	...
	cache_size = max(cache_size, ARRAY_SIZE(incall_rcv_reg));   //add incall_rcv_reg
...
static int wm8994_open(struct inode *inode, struct file *file)
{
	printk("%s\n", __func__);
	
	return 0;
}
int wm8994_release(struct inode *inode, struct file *file)
{
	printk("%s\n", __func__);
...
static ssize_t wm8994_misc_read(struct file *file, char __user * buf, size_t size, loff_t *offset)
{
...
unsigned int char2hex(char *buf)
{
	unsigned int hex = 0;
	int i=0;
...
static ssize_t wm8994_misc_write (struct file *file, const char __user *buf, size_t size, loff_t *offset)
{
	unsigned long rc;
	unsigned int reg;
//	unsigned int dest;
	...
	dprintk("%s size %d\n",__func__,size);
	dprintk("%s, %s",__func__,data);
	if(data[0] == 'r')
	{
		int rc = 0;
		
		reg = (unsigned int)char2hex(&data[1]);
		dprintk("reg  0x%x\n",reg);
		
		rc = wm8994_read(codec_misc, reg);
		if(rc < 0){
	...
	else if(data[0] == 'w')
	{
		...
		reg = (unsigned int)char2hex(&data[1]);
		val = char2hex(&data[6]);
		dprintk("reg  0x%x  val 0x%x\n",reg,val);
		
		rc = wm8994_write(codec_misc, reg,val); 
		if(rc < 0){
			printk("i2c write error %d\n",rc);
		}
		rc = wm8994_read(codec_misc, reg);
		dprintk("readback val:0x%x, write val:0x%x\n",rc,val);
	}
	else if(data[0] == 'd')
	{
		int i=0;
		...
		for(i=0;i<750;i++)
		{
			rc = wm8994_read(codec_misc, i);
			if(rc < 0){
				printk("i2c read error %ld\n",rc);
			}
			if(i%10 == 0)
				printk("\n[%d] 0x%x:0x%lx - ",i/10,i,rc);
			else
				printk(" 0x%x:0x%lx -",i,rc);			
		}
		dprintk("\n");
	}
	else if(data[0] == 'p')
	{
		...
		for(i=0;i<ARRAY_SIZE(incall_hp_reg);i++)
		{
			rc = wm8994_read(codec_misc, incall_hp_reg[i].reg);
			if(rc < 0){
				printk("i2c read error %ld\n",rc);
			}
				printk("\n {0x%x , 0x%lx},",incall_hp_reg[i].reg,rc);
		}
		dprintk("\n");
	}
	else if(data[0] == 't')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg);i++)
		{
			rc = wm8994_read(codec_misc, normal_hp_reg[i].reg);
			if(rc < 0){
				printk("i2c read error %ld\n",rc);
			}
				printk("\n {0x%x , 0x%lx},",normal_hp_reg[i].reg,rc);
		}
		dprintk("\n");
	}
	...
	else if(data[0] == 'h')
	{
		...
		for(i=0;i<ARRAY_SIZE(incall_rcv_reg);i++)
		{
			rc = wm8994_write(codec_misc, incall_rcv_reg[i].reg, incall_rcv_reg[i].val);
			if(rc < 0){
				printk("i2c read error %ld\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",incall_rcv_reg[i].reg, incall_rcv_reg[i].val);
		}
		dprintk("\n");
	}
	else if(data[0] == 'f')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg[i].reg, normal_hp_reg[i].val);
			if(rc < 0){
				printk("i2c read error %ld\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg[i].reg, normal_hp_reg[i].val);
		}
		dprintk("\n");
	}
	#if 0
	else if(data[0] == '0')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg0);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg0[i].reg, normal_hp_reg0[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg0[i].reg, normal_hp_reg0[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '1')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg1);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg1[i].reg, normal_hp_reg1[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg1[i].reg, normal_hp_reg1[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '2')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg2);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg2[i].reg, normal_hp_reg2[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg2[i].reg, normal_hp_reg2[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '3')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg3);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg3[i].reg, normal_hp_reg3[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg3[i].reg, normal_hp_reg3[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '4')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg4);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg4[i].reg, normal_hp_reg4[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg4[i].reg, normal_hp_reg4[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '5')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg5);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg5[i].reg, normal_hp_reg5[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg5[i].reg, normal_hp_reg5[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '6')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg6);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg6[i].reg, normal_hp_reg6[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg6[i].reg, normal_hp_reg6[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '7')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg1);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg7[i].reg, normal_hp_reg7[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg7[i].reg, normal_hp_reg7[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '8')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg1);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg8[i].reg, normal_hp_reg8[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg8[i].reg, normal_hp_reg8[i].val);
		}
		printk("\n",rc);
	}
	else if(data[0] == '9')
	{
		...
		for(i=0;i<ARRAY_SIZE(normal_hp_reg1);i++)
		{
			rc = wm8994_write(codec_misc, normal_hp_reg9[i].reg, normal_hp_reg9[i].val);
			if(rc < 0){
				printk("i2c read error %d\n",rc);
			}
				printk("\n wm8994_write: {0x%x , 0x%x},",normal_hp_reg9[i].reg, normal_hp_reg9[i].val);
		}
		printk("\n",rc);
	}
	#endif
...
//For read and write vm8994 registers---tab.wang 2012-8-5  START//
long wm8994_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int rc = 0;
	char *data;
	unsigned int reg;
	switch (cmd) {
	case WM8994_READ_REG:
		data = kmalloc(4 * sizeof(char),GFP_KERNEL);
		rc = copy_from_user((void *)data, (void *)arg, 4 * sizeof(char));
		reg = (unsigned int)char2hex(&data[0]);
		dprintk("reg  0x%x\n",reg);
		...
		rc = copy_to_user((void *)arg, &rc, sizeof(rc));
		break;

	case WM8994_WRITE_REG:
		data = kmalloc(8 * sizeof(char),GFP_KERNEL);
		rc = copy_from_user((void *)data, (void *)arg, 8 * sizeof(char));
		reg = (unsigned int)char2hex(&data[0]);
		unsigned int val ;

		val = (unsigned int)char2hex(&data[4]);
		dprintk("reg  0x%x  val 0x%x\n",reg,val);
...
//For read and write vm8994 registers---tab.wang 2012-8-5  END//

static const struct file_operations wm8994_fileops = {
	.owner = THIS_MODULE,
	.open = wm8994_open,
	.release = wm8994_release,
	.read = wm8994_misc_read,
	.write = wm8994_misc_write,
	.unlocked_ioctl = wm8994_ioctl,
};


static struct miscdevice wm8994_device = {
...
#ifdef CALL_LOCK_TEMP
	wake_lock_init(&call_wake_lock, WAKE_LOCK_SUSPEND, "call_lock");
#endif
...

{
	...
	rc = misc_register(&wm8994_device);


	codec_misc = codec;
	
	if (rc) {
		printk("%s: wm8994_device register failed\n", __func__);
	}
}
	...
	/*add by TerryHuang for register 0x620 which must be 0x0, or will make no sound*/
	...
	case WM8994:
		...
		case 2:
		case 3:
			//wm8994->hubs.dcs_codes = -5;//it will make pop noise while open and close music.
