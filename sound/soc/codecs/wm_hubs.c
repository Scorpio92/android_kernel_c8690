		    ...
		    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_update_bits(codec, WM8993_LEFT_OUTPUT_VOLUME, 1<<6, 0<<6);  //mute hp
		snd_soc_update_bits(codec, WM8993_RIGHT_OUTPUT_VOLUME, 1<<6, 0<<6); //mute hp
		snd_soc_update_bits(codec, WM8993_ANALOGUE_HP_0, reg, reg);
		...
		//snd_soc_write(codec, WM8993_ANALOGUE_HP_0, reg);
		snd_soc_update_bits(codec, WM8993_ANALOGUE_HP_0, reg, reg);
		snd_soc_update_bits(codec, WM8993_LEFT_OUTPUT_VOLUME, 1<<6, 1<<6); //umute hp
		snd_soc_update_bits(codec, WM8993_RIGHT_OUTPUT_VOLUME, 1<<6, 1<<6); //umute hp
		break;
...
extern int late_enable_ev(struct snd_soc_dapm_widget *w,
			  struct snd_kcontrol *kcontrol, int event);

static int left_output_mixer_ev(struct snd_soc_dapm_widget *w,
		      struct snd_kcontrol *kcontrol, int event)
{
...
SND_SOC_DAPM_LINE("IN2LN", NULL), /* wenpin.cui: for fm */
SND_SOC_DAPM_LINE("IN2LP:VXRN", NULL), /* wenpin.cui: for fm */
SND_SOC_DAPM_INPUT("IN1RN"),
SND_SOC_DAPM_INPUT("IN1RP"),
SND_SOC_DAPM_INPUT("IN2RN"),
SND_SOC_DAPM_INPUT("IN2RP:VXRP"),

/*we control bias in switch_gpio.c, here is dummy for dapm -- tab.wang*/
//SND_SOC_DAPM_MICBIAS("MICBIAS2", WM8993_POWER_MANAGEMENT_1, 5, 0),
SND_SOC_DAPM_MICBIAS("MICBIAS2", SND_SOC_NOPM, 0, 0),
...
SND_SOC_DAPM_MIXER_E("Left Output Mixer", WM8993_POWER_MANAGEMENT_3, 5, 0,
		   left_output_mixer, ARRAY_SIZE(left_output_mixer), 
		   left_output_mixer_ev, SND_SOC_DAPM_PRE_PMU),
