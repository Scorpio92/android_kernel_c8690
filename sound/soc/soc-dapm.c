...
static int snd_soc_dapm_suspend_check(struct snd_soc_dapm_widget *widget)
{
	...
	case SNDRV_CTL_POWER_D3cold:
		if (widget->ignore_suspend && tc4_get_call_flg())
			dev_dbg(widget->dapm->dev, "%s ignoring suspend\n",
				widget->name);
		return widget->ignore_suspend && tc4_get_call_flg();
	default:
		return 1;
	}
}
...
void soc_dapm_stream_event(struct snd_soc_dapm_context *dapm,
...
EXPORT_SYMBOL_GPL(soc_dapm_stream_event);
